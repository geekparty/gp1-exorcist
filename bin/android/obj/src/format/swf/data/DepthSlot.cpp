...
HX_STACK_PUSH("DepthSlot::new","format/swf/data/DepthSlot.hx",19);
{
	...
	this->symbol = symbol;
	HX_STACK_LINE(24)
	this->attributes = Array_obj< ::format::swf::data::DisplayAttributes >::__new();
	HX_STACK_LINE(25)
...
Void DepthSlot_obj::move( int frame,::neash::geom::Matrix matrix,::neash::geom::ColorTransform colorTransform,Dynamic ratio){
{
...
int DepthSlot_obj::findClosestFrame( int hintFrame,int frame){
	...
	if (((last >= this->attributes->length))){
		...
		if (((last > (int)0))){
			HX_STACK_LINE(40)
			if (((this->attributes->__get((last - (int)1))->frame > frame))){
				HX_STACK_LINE(42)
				last = (int)0;
			}
		}
	...
	HX_STACK_LINE(50)
	{
		...
		while(((_g1 < _g))){
			...
			HX_STACK_LINE(52)
			if (((this->attributes->__get(i)->frame > frame))){
				HX_STACK_LINE(52)
				return last;
			}
			HX_STACK_LINE(58)
...
Dynamic DepthSlot_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	...
	case 8:
		...
		break;
	case 10:
		if (HX_FIELD_EQ(inName,"attributes") ) { attributes=inValue.Cast< Array< ::format::swf::data::DisplayAttributes > >(); return inValue; }
		break;
	case 15:
...
{
	Static(__mClass) = hx::RegisterClass(HX_CSTRING("format.swf.data.DepthSlot"), hx::TCanCast< DepthSlot_obj> ,sStaticFields,sMemberFields,
