...
Dynamic ShapeEdge_obj::asCommand( ){
	...
	HX_STACK_THIS(this);
	HX_STACK_LINE(734)
	Array< ::format::swf::symbol::ShapeEdge > _g = Array_obj< ::format::swf::symbol::ShapeEdge >::__new().Add(hx::ObjectPtr<OBJ_>(this));		HX_STACK_VAR(_g,"_g");
	HX_STACK_LINE(736)
	if ((this->isQuadratic)){

		HX_BEGIN_LOCAL_FUNC_S1(hx::LocalFunc,_Function_2_1,Array< ::format::swf::symbol::ShapeEdge >,_g)
		Void run(::neash::display::Graphics gfx){
			HX_STACK_PUSH("*::_Function_2_1","format/swf/symbol/Shape.hx",738);
			HX_STACK_ARG(gfx,"gfx");
			{
				HX_STACK_LINE(738)
				gfx->curveTo(_g->__get((int)0)->cx,_g->__get((int)0)->cy,_g->__get((int)0)->x1,_g->__get((int)0)->y1);
			}
			return null();
		}
		...
		Void run(::neash::display::Graphics gfx){
			...
			{
				HX_STACK_LINE(746)
				gfx->lineTo(_g->__get((int)0)->x1,_g->__get((int)0)->y1);
			}
			return null();
		}
		HX_END_LOCAL_FUNC1((void))
...
{
	Static(__mClass) = hx::RegisterClass(HX_CSTRING("format.swf.symbol.ShapeEdge"), hx::TCanCast< ShapeEdge_obj> ,sStaticFields,sMemberFields,
