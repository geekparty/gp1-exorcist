		...
		while(((_g < numGlyphs))){
			...
			while((true)){
				...
				if ((!(edge))){
					...
					Void run(::neash::display::Graphics g,::neash::geom::Matrix m){
...
Void Font_obj::renderGlyph( ::neash::display::Graphics graphics,int index,::neash::geom::Matrix matrix){
{
...
Float Font_obj::renderCharacter( ::neash::display::Graphics graphics,int code,::neash::geom::Matrix matrix){
...
{
	Static(__mClass) = hx::RegisterClass(HX_CSTRING("format.swf.symbol.Font"), hx::TCanCast< Font_obj> ,sStaticFields,sMemberFields,
