...
{
	HX_STACK_LINE(35)
	this->textFormat = ::neash::text::TextFormat_obj::__new(null(),null(),null(),null(),null(),null(),null(),null(),null(),null(),null(),null(),null());
	HX_STACK_LINE(37)
	...
	if ((hasFont)){
		...
		HX_STACK_LINE(67)
		{
			::format::swf::symbol::Symbol _switch_1 = (swf->getSymbol(fontID));
...
Void EditText_obj::apply( ::neash::text::TextField textField){
{
		HX_STACK_PUSH("EditText::apply","format/swf/symbol/EditText.hx",130);
		HX_STACK_THIS(this);
		HX_STACK_ARG(textField,"textField");
		HX_STACK_LINE(132)
		textField->nmeSetWordWrap(this->wordWrap);
		HX_STACK_LINE(133)
		textField->nmeSetMultiline(this->multiline);
		HX_STACK_LINE(134)
		textField->nmeSetWidth(this->rect->width);
		HX_STACK_LINE(135)
		textField->nmeSetHeight(this->rect->height);
		HX_STACK_LINE(136)
		textField->nmeSetDisplayAsPassword(this->displayAsPassword);
		HX_STACK_LINE(138)
		if (((this->maxChars > (int)0))){
			HX_STACK_LINE(138)
			textField->nmeSetMaxChars(this->maxChars);
		}
		HX_STACK_LINE(144)
		textField->nmeSetBorder(this->border);
		HX_STACK_LINE(145)
		textField->nmeSetBorderColor((int)0);
		HX_STACK_LINE(147)
		if ((this->readOnly)){
			HX_STACK_LINE(147)
			textField->nmeSetType(::neash::text::TextFieldType_obj::DYNAMIC_dyn());
		}
		else{
			HX_STACK_LINE(151)
			textField->nmeSetType(::neash::text::TextFieldType_obj::INPUT_dyn());
		}
		HX_STACK_LINE(157)
		if ((this->autoSize)){
			HX_STACK_LINE(157)
			textField->nmeSetAutoSize(::neash::text::TextFieldAutoSize_obj::CENTER_dyn());
		}
		else{
			HX_STACK_LINE(161)
			textField->nmeSetAutoSize(::neash::text::TextFieldAutoSize_obj::NONE_dyn());
		}
		HX_STACK_LINE(167)
		textField->setTextFormat(this->textFormat,null(),null());
		HX_STACK_LINE(168)
		textField->nmeSetSelectable(!(this->noSelect));
		HX_STACK_LINE(172)
		if ((this->html)){
			HX_STACK_LINE(172)
			textField->nmeSetHTMLText(this->text);
		}
		else{
			HX_STACK_LINE(176)
			textField->nmeSetText(this->text);
		}
	}
return null();
}
...
Dynamic EditText_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	...
	case 9:
		...
		break;
	case 10:
		if (HX_FIELD_EQ(inName,"textFormat") ) { textFormat=inValue.Cast< ::neash::text::TextFormat >(); return inValue; }
		break;
	case 11:
...
{
	Static(__mClass) = hx::RegisterClass(HX_CSTRING("format.swf.symbol.EditText"), hx::TCanCast< EditText_obj> ,sStaticFields,sMemberFields,
